...
function renderTimelineView(schedule) {
    const timelineHTML = `
        <ul class="timeline" id="timeline-list">
            ...
            `).join('')}
            <div class="current-time-indicator" id="current-time-indicator" style="display: none;">
                <span class="current-time-text" id="current-time-text"></span>
            </div>
        </ul>
    `;
...
function updateCurrentTimeIndicator() {
    ...
    if (!state.currentTime || !indicator || !timeText || !timeline || !state.schedule || state.schedule.length < 2) {
    ...
    const hours = String(state.currentTime.getHours()).padStart(2, '0');
    const minutes = String(state.currentTime.getMinutes()).padStart(2, '0');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('mousemove', (e) => {
        const target = e.target;
        if (target && target.classList.contains('clock-sector') && target.dataset.tooltip) {
